...
# trace learning rate
# seem that AdamOptimizer does not change the learning_rate var
...
def new_var_dict():
    ...
    ret['learning_rate'] = tf.Variable(0.001, trainable=False)
    return ret
...
#     mid = mid + ( tf.abs( tf.reduce_mean(var_dict['b5']) ) * tf.constant(L2_WEIGHT) )
    ...
    mid = tf.train.AdamOptimizer(learning_rate=var_dict['learning_rate']).minimize(mid,var_list=var_dict.values())
    train = mid
...
class DeepLearn(object):
    ...
    def do_train(self):
        ...
        _, loss, score_diff, learning_rate = self.sess.run([self.train,self.loss,self.score_diff,self.var_dict['learning_rate']],feed_dict=feed_dict)
