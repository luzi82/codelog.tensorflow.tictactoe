...
def new_var_dict():
    ...
    ret['global_step'] = tf.Variable(0, trainable=False)
    return ret
...
#     mid = mid + ( tf.abs( tf.reduce_mean(var_dict['b5']) ) * tf.constant(L2_WEIGHT) )
    ...
    learning_rate = tf.train.exponential_decay(
        arg_dict['starter_learning_rate'],
        var_dict['global_step'],
        arg_dict['learning_rate_decay_steps'],
        arg_dict['learning_rate_decay_rate']
    )
    mid = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(mid,var_list=var_dict.values(),global_step=var_dict['global_step'])
    train = mid
    
    return train, loss, score_diff, learning_rate
...
class DeepLearn(object):
    ...
    def __init__(self,arg_dict):
        ...
        self.train, self.loss, self.score_diff, self.learning_rate = get_train(self.train_ph_dict,self.var_dict,self.var_ph_dict,self.arg_dict)
    ...
    def do_train(self):
        ...
        _, loss, score_diff, learning_rate = self.sess.run([self.train,self.loss,self.score_diff,self.learning_rate],feed_dict=feed_dict)
        ...
        if self.train_count % 1000 == 0:
            ...
            logging.info('CLPNAVGR save session: {}, loss: {}, learning_rate: {}, time: {}, time_i: {}'.format(output_file_name,loss,learning_rate,int((time.time()-self.timestamp)*1000),int((timestamp_last-self.timestamp_last)*1000)))
