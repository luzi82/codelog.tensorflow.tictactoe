...
def get_q(state_ph,var_dict,dropout_keep_prob):
    mid = state_ph
    ...
    mid = mid + var_dict['b9']
    mid = tf.nn.dropout(mid,dropout_keep_prob)
...
def get_train(train_ph_dict,var_dict,var_ph_dict,arg_dict,dropout_keep_prob):
    mid0 = tf.one_hot(train_ph_dict['choice_0'], 9, axis=-1, dtype=tf.float32)
    mid0 = mid0 * get_q(train_ph_dict['state_0'],var_dict,dropout_keep_prob)
    mid0 = tf.reduce_sum(mid0, reduction_indices=[1])

    mid1 = get_q(train_ph_dict['state_1'],var_ph_dict,dropout_keep_prob)
...
class DeepLearn(object):
    ...
    def __init__(self,arg_dict):
        ...
        self.score, self.weight, self.train_choice, self.choice_cal = get_train_choice(self.choice_state,self.var_dict,self.random_t,self.mask,self.dropout_keep_prob)
        
        # train
        self.train_ph_dict = new_train_ph_dict()
        self.train, self.loss, self.score_diff = get_train(self.train_ph_dict,self.var_dict,self.var_ph_dict,self.arg_dict,self.dropout_keep_prob)
    ...
    def cal_choice(self, state_0, mask, train_enable):
        if train_enable and random.random() < self.arg_dict['random_move_chance']:
            ...
            score, choice_0, weight = self.sess.run([self.score, self.train_choice, self.weight],feed_dict={self.choice_state:[state_0],self.mask:[mask],self.dropout_keep_prob:1.0})
            ...
            score, choice_0 = self.sess.run([self.score, self.choice_cal],feed_dict={self.choice_state:[state_0],self.mask:[mask],self.dropout_keep_prob:1.0})
            score = score[0].tolist()
            choice_0 = choice_0.tolist()[0]
    ...
    def do_train(self):
        ...
        feed_dict[self.dropout_keep_prob] = self.arg_dict['dropout_keep_prob']
